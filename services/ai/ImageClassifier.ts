...
export type ClassifyResult = { suggested?: { tipo: TipoResiduo; confidence: number }, raw: Prediction[] };

async function ensureReady() {
  if (Platform.OS === 'web') throw new Error('IA indisponível no web build.');
  if (!inited) {
    await tf.ready();
    if (tf.getBackend() !== 'rn-webgl') {
      await tf.setBackend('rn-webgl'); // backend GPU do RN
      await tf.ready();
    }
...
export async function classifyImage(uri: string, topK = 5): Promise<ClassifyResult> {
  ...
  const preds = await mobilenet.classify(img, topK) as Array<{ className: string; probability: number }>;
  img.dispose?.();

  const raw: Prediction[] = preds.map(p => ({ label: p.className, prob: p.probability }));

  const scores: Record<TipoResiduo, number> = { vidro: 0, papel: 0, plastico: 0, metal: 0, pilhas: 0, organico: 0 };
  for (const p of raw) {
  ...
  (Object.keys(scores) as TipoResiduo[]).forEach(t => { if (scores[t] > best.score) best = { tipo: t, score: scores[t] }; });
  const suggested = best.tipo ? { tipo: best.tipo, confidence: Math.min(1, best.score) } : undefined;
  return { suggested, raw };
}

export async function warmupClassifier() { try { await ensureReady(); } catch {} }
